...
import { changePassword, getProfileSummary, type ProfileSummaryResponse } from "@/lib/auth-api";
...
export default function HoSoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getProfileSummary(token), getTopics()])
      .then(([summary, topicList]) => {
  ...
  if (loading) {
    return (
      <main className="mx-auto w-full max-w-[1200px] px-6 py-8 md:px-10">
        <div className="rounded-2xl border border-primary/10 bg-surface p-8 text-sm text-muted">
          Đang tải hồ sơ...
        </div>
      </main>
    );
        ...
        <>
          <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <article className="rounded-2xl border border-primary/10 bg-surface p-5 shadow-sm">
              ...
              <p className="mt-2 text-xl font-bold">{profileData.user.username}</p>
              <p className="text-sm text-muted">{profileData.user.email ?? "Chưa cập nhật email"}</p>
              <p className="mt-1 text-sm text-muted">
                Vai trò: {ROLE_LABEL[profileData.user.role_id ?? 3] ?? "Học sinh"}
              </p>
            </article>

            <article className="rounded-2xl border border-primary/10 bg-surface p-5 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-muted">Số bài luyện tập</p>
              <p className="mt-2 text-3xl font-black text-primary">{profileData.stats.so_bai_luyen_tap}</p>
            </article>

            <article className="rounded-2xl border border-primary/10 bg-surface p-5 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-muted">Tổng điểm KN</p>
                  ...
                  <tbody className="divide-y divide-primary/10">
                    {profileData.history.map((item) => (
                      <tr key={item.id} className="hover:bg-primary/5">
                        ...
                        <td className="px-4 py-3">#{item.luyen_tap_dich_id}</td>
                        <td className="px-4 py-3">{LEVEL_LABEL[item.level] ?? item.level}</td>
                        <td className="px-4 py-3 text-sm text-muted">
